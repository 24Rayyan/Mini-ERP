...
class FinancialReportController extends Controller
{
    ...
    /**
     * Halaman Lampiran Khusus: Daftar Nominatif Biaya Entertainment & Promosi SPT Tahunan DJP
     */
    public function entertainmentNominative(Request $request)
    {
        $year = (int) ($request->get('year', date('Y')));
        $month = $request->filled('month') && $request->month !== 'all' ? (int) $request->month : null;

        $nominativeList = $this->reportService->getEntertainmentNominativeList($year, $month);
        $totalAmount = $nominativeList->sum(fn($item) => $item->transaction?->amount ?? 0);
        $setting = Setting::first();
        ...
        return view('reports.entertainment_nominative', compact(
            'nominativeList',
            'totalAmount',
            'setting',
            'year',
    ...
    /**
     * Export Lampiran Khusus Daftar Nominatif Entertainment ke PDF Format Resmi DJP
     */
    public function exportEntertainmentPdf(Request $request)
    {
        $year = (int) ($request->get('year', date('Y')));
        $month = $request->filled('month') && $request->month !== 'all' ? (int) $request->month : null;
        ...
        $pdf = Pdf::loadView('exports.entertainment_nominative_pdf', compact('nominativeList', 'totalAmount', 'setting', 'year', 'month'))
            ->setPaper('a4', 'landscape');

        $periodSlug = $month ? "Bulan_{$month}_{$year}" : "Tahun_{$year}";
        $fileName = "Lampiran_SPT_Nominatif_Entertainment_{$periodSlug}.pdf";

        return $pdf->download($fileName);
    }

    /**
     * Export Lampiran Khusus Daftar Nominatif Entertainment ke Excel (.xls)
     */
    public function exportEntertainmentExcel(Request $request)
    {
        $year = (int) ($request->get('year', date('Y')));
        $month = $request->filled('month') && $request->month !== 'all' ? (int) $request->month : null;
        ...
        $periodSlug = $month ? "Bulan_{$month}_{$year}" : "Tahun_{$year}";
        $fileName = "Lampiran_SPT_Nominatif_Entertainment_{$periodSlug}.xls";

        $content = view('exports.entertainment_nominative_excel', compact('nominativeList', 'totalAmount', 'setting', 'year', 'month'))->render();
